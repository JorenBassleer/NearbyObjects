...
const speedMultiplier = shallowRef(1);
...
watch(astroidRef, (model) => {
  ...
  onLoop(({ delta, elapsed }) => {
    if (model.value) {
      ...
      // const speedAdjustment = delta * orbitSpeed * speedMultiplier.value;
      /* eslint-disable no-param-reassign */
      model.value.rotation.y += Math.sin(delta * orbitSpeed);
      model.value.rotation.z += Math.sin(delta * orbitSpeed);
      model.value.position.x = props.positionEarth.x + props.asteroid.close_approach_data[0].miss_distance.lunar * Math.sin(angle) * speedMultiplier.value;
      model.value.position.z = props.positionEarth.z + props.asteroid.close_approach_data[0].miss_distance.lunar * Math.cos(angle) * speedMultiplier.value;
...
watch(() => props.isFocused, (newVal) => {
  if (newVal) {
    gsap.to(speedMultiplier, { value: 0, duration: 2 });
  } else {
    gsap.to(speedMultiplier, { value: 1, duration: 2 });
  }
});
</script>
